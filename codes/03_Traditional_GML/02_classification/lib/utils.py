...
def compute_test_kernel(Xtrain,Xtest,type_kernel,parameters=None):
    ...
    if type_kernel=='polynomial':
        ...
        if isinstance(X, np.ndarray)==False:
            X = X.toarray()
        meanXtrain = np.mean(X,axis=0)
        X -= meanXtrain
        X = ( X.T / np.sqrt(np.sum(X**2,axis=1)+1e-10) ).T # Projection on the sphere, i.e. ||x_i||_2 = 1
        Xtrain = X
        ...
        D = Xtrain.dot(Xtest.T)

        # Polynomial Kernel
        KXtest = pow( np.abs( a* D + b ) , c )
